...
def log(stage: str, status: str, message: str): print(f"[{status}] {stage}: {message}", flush=True)
...
def build_messages(result: dict, today: str) -> list[str]:
    ...
    for s in top:
        ...
        timeline += f"\n<b>{s.get('headline','')}</b>\n" + "".join(f"• {e.get('date','')} — {e.get('headline','')}\n" for e in events[:6])
    ...
    msgs.append(text)
    text = "🔄 <b>REVISION</b>\n" + "".join(f"\n<b>{x.get('topic','')}</b>\nQ: {x.get('question','')}\nA: {x.get('answer','')}\n" for x in result.get("revision", [])[:5]); msgs.append(text)
    text = "❓ <b>DAILY QUIZ</b>\n" + "".join(f"\n<b>Q{i}.</b> {q.get('question','')}\nAnswer: {q.get('answer','')}\n" for i, q in enumerate(result.get("quiz", [])[:5], 1)); msgs.append(text)
    return msgs
...
def main():
    ...
    try:
        ...
        historical = research_stories(selected); historical["daily_learning"] = learning; log("Historical research", "PASS", f"Research completed for {len(historical)} selected stories")
        result = generate_briefing(selected, [a.__dict__ for a in articles], previous, today, historical); log("AI final briefing", "PASS", f"Generated {len(result.get('top_stories', []))} stories")
        ...
        print(json.dumps({"date": today, "articles": len(articles), "selected": len(selected), "stories": len(result.get("top_stories", []))}, indent=2)); log("FINAL", "PASS", "Morning knowledge pipeline completed")
    except Exception as exc:
