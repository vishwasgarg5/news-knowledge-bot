...
def history_snapshot():
    return {
        "knowledge_cards": read_rows(DATA / "knowledge_cards.csv")[-80:],
        "vocabulary": read_rows(DATA / "vocabulary.csv")[-80:],
        "people": read_rows(DATA / "people.csv")[-40:],
        "places": read_rows(DATA / "places.csv")[-40:],
        "story_timeline": read_rows(DATA / "story_timeline.csv")[-100:],
        "current_affairs": read_rows(DATA / "current_affairs.csv")[-80:],
    }
...
def build_messages(result: dict, today: str) -> list[str]:
    ...
    for s in top:
        for p in s.get("people", [])[:2]:
            text += f"\n👤 <b>{p.get('name','')}</b> — {p.get('role','')}\n{p.get('background','')}\n"
        for p in s.get("places", [])[:2]:
            text += f"\n📍 <b>{p.get('name','')}</b> — {p.get('location','')}\n{p.get('background','')}\n"
        for c in s.get("concepts", [])[:1]:
            text += f"\n💡 <b>{c.get('topic','')}</b>\n{c.get('explanation','')}\nRelated: {c.get('related_topics','')}\n"
    for c in result.get("connect_the_dots", [])[:5]:
        text += f"\n🔗 <b>{c.get('chain','')}</b>\n{c.get('explanation','')}\n"
    msgs.append(text)
    ...
    text = "🔄 <b>REVISION</b>\n"
    for x in result.get("revision", [])[:5]:
        text += f"\n<b>{x.get('topic','')}</b>\nQ: {x.get('question','')}\nA: {x.get('answer','')}\n"
    msgs.append(text)

    text = "❓ <b>DAILY QUIZ</b>\n"
    for i, q in enumerate(result.get("quiz", [])[:5], 1):
...
def main():
    ...
    log("Credentials", "PASS", "No paid AI API key required")
    ...
    try:
        # Stage 1: cheap/compact AI selection instead of sending hundreds of full articles.
        selected = select_stories([a.__dict__ for a in articles], top_n=min(12, limits.get("top_stories", 12)))
        log("AI selection", "PASS", f"Reduced {len(articles)} articles to {len(selected)} priority stories")

        # Stage 2: research only selected stories. This avoids dozens of unnecessary web requests.
        historical = research_stories(selected)
        historical["daily_learning"] = learning
        log("Historical research", "PASS", f"Research completed for {len(historical)} selected stories")

        # Stage 3: one compact final generation call using only selected evidence + small memory snapshot.
        result = generate_briefing(selected, [a.__dict__ for a in articles], previous, today, historical)
        ...
        print(json.dumps({"date": today, "articles": len(articles), "selected": len(selected), "stories": len(result.get("top_stories", []))}, indent=2))
        log("FINAL", "PASS", "Morning knowledge pipeline completed")
    except Exception as exc:
