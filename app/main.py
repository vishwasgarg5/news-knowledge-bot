...
def topic_id(headline):
    words=[w for w in re.findall(r"[A-Za-z]{4,}",str(headline).lower()) if w not in {"today","latest","says","after","amid","over","news","india"}];return hashlib.sha1((" ".join(words[:10]) or str(headline).lower()).encode()).hexdigest()[:16]
...
def _vocab_words(story):
    text=f"{story.get('what','')} {story.get('why','')} {story.get('background','')}";words=re.findall(r"\b[A-Za-z]{7,}\b",text);stop={"because","important","government","development","according","following","between","including","international","situation","background","provided","sources"};return list(dict.fromkeys(w.lower() for w in words if w.lower() not in stop))[:3]
