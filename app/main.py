...
def yesterday_changes(stories, history, today):
    ...
    for s in stories:
        h = re.sub(r"\W+", " ", str(s.get("headline", "")).lower()).strip()
        s["change_since_yesterday"] = "New topic today" if h not in prev else "Continuing story"
    return stories
...
def persist(result, today):
    ...
    existing = {r.get("story_id") for r in history}
    cards = read_rows(DATA / "knowledge_cards.csv")
    cardmap = {r.get("topic_id"): r for r in cards}
    timeline = read_rows(DATA / "story_timeline.csv")
    timeline_keys = {(r.get("story_id"), r.get("date")) for r in timeline}
    affairs = read_rows(DATA / "current_affairs.csv")
    affair_keys = {(r.get("topic"), r.get("date")) for r in affairs}
    quizzes = read_rows(DATA / "quiz_history.csv")
    quiz_keys = {r.get("question") for r in quizzes}
    for s in stories:
        sid = s.get("story_id"); url = (s.get("sources") or [""])[0]; headline = s.get("headline", ""); tid = topic_id(headline)
        old = cardmap.get(tid); first = old.get("first_seen", today) if old else today; count = int(old.get("review_count", 0) or 0) if old else 0
        if sid and sid not in existing:
            append_rows(DATA/"news_history.csv", [{"date":today,"story_id":sid,"headline":headline,"source":"","url":url,"category":s.get("category"),"importance":s.get("importance",0)}], HEADERS["news_history.csv"]); existing.add(sid)
        if (sid, today) not in timeline_keys:
            append_rows(DATA/"story_timeline.csv", [{"story_id":sid,"date":today,"headline":headline,"event":s.get("latest_update",s.get("what","")),"importance":s.get("importance",0),"source":"","url":url}], HEADERS["story_timeline.csv"]); timeline_keys.add((sid,today))
        if (headline, today) not in affair_keys:
            append_rows(DATA/"current_affairs.csv", [{"date":today,"topic":headline,"category":s.get("category",""),"summary":s.get("what",s.get("latest_update","")),"why_important":s.get("why_important",""),"source_url":url}], HEADERS["current_affairs.csv"]); affair_keys.add((headline,today))
        ...
        if q not in quiz_keys:
            append_rows(DATA/"quiz_history.csv", [{"date":today,"question":q,"answer":answer,"topic":headline,"difficulty":"easy"}], HEADERS["quiz_history.csv"]); quiz_keys.add(q)
...
def main():
    ...
    cfg = load_sources(); limits = cfg.get("limits",{}); articles = collect(cfg.get("sources",{}), limits.get("max_articles_per_source",30), limits.get("max_total_articles",500)); log("News collection","PASS" if articles else "FAIL",f"Collected {len(articles)} articles")
    ...
    selected = select_stories([a.__dict__ for a in articles], top_n=min(12,limits.get("top_stories",12))); selected = yesterday_changes(selected, previous.get("news_history",[]), today); log("Story selection","PASS",f"Selected {len(selected)} priority stories")
    ...
    result = generate_briefing(selected,[a.__dict__ for a in articles],previous,today,historical); result["research_stats"] = stats
    review_rows = read_rows(DATA/"quiz_history.csv"); result["review_questions"] = [r.get("question","") for r in review_rows[-5:][::-1]]
    log("AI final briefing","PASS",f"Generated {len(result.get('top_stories',[]))} stories")
    persist(result,today); log("CSV persistence","PASS","Long-term news, topic timeline, current affairs and review memory updated")
    if tg:
